...
namespace myos {
	namespace net {
		...
		bool InternetProtocolHandler::OnInternetProtocolRecevied(uint32_t srcIP_BE, uint32_t dstIP_BE, uint8_t* internetProtocolPayload, uint32_t size){
			return false;
		}
		...
		InternetProtocolProvider::InternetProtocolProvider(EtherFrameProvider* backend,
			...
			uint32_t gatewayIP,
			uint32_t subnetMask):EtherFrameHandler(backend,0x800) {
			for (int i = 0; i < 255; i++) {
				handlers[i] = nullptr;
			}
			this->arp = arp;
		...
		void InternetProtocolProvider::Send(uint32_t dstIP_BE, uint8_t protocol, uint8_t* data, uint32_t size) {
			uint8_t* buffer = (uint8_t*)kernel::MemoryManager::activeMemoryManager->malloc(sizeof(InternetProtocolV4Message) + size);
